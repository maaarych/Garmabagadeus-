...
class Track:
        ...
        if self.safety:
            if isinstance(order, Coffee) or isinstance(order, CustomCoffee):
                if order.name in self.MENU:
                    ...
                    self.total_milk += order.milk
                    self.total_beans += order.espresso
                    return 'Done!'
    ...
    def milk(self):
        """getter"""
        return int(self.__milk - self.total_milk) if self.total_milk < self.__milk else 0
...
class FlavorMixin:
    """class flavor mixing"""
    __slots__ = ['sugar', 'cinammon', 'syrup', 'flavor']

    def add_flavor(self, sugar: int = 0, cinammon: bool = False, syrup: str = False):
        """adds flavor"""
        if self.is_paid:
            self.sugar = sugar * self.count if self.count > 1 else sugar
...
class CustomCoffee(Coffee, FlavorMixin):
    ...
    def __str__(self) -> str:
        ...
        elif self.is_paid and self.flavor is False:
            return f'Preparing {self.count} {self.name}...'
        elif self.is_paid and self.flavor is True:
    ...
    def __eq__(self, cof: object) -> bool:
        ...
        if isinstance(cof, Coffee):
            return (self.name, self.count, self.flavor) == (cof.name, cof.count, False)
        return (self.name, self.count, self.sugar, self.cinammon, self.syrup) == \
        (cof.name, cof.count, cof.sugar, cof.cinammon, cof.syrup)
